...
from model import (
    ...
    expected_passengers_per_station,
)
...
def test_compute_alphas_with_lists():
    n_on = [10, 5, 2]
    n_off = [3, 8, 1]
    expected = jnp.array([1.0, 0.2, 6 / 7])
    assert jnp.allclose(compute_alphas(n_on, n_off), expected)
...
def test_expected_passengers_with_lists():
    n_on = [10, 0]
    n_off = [0, 4]
    arr_result = expected_passengers_per_station(jnp.array(n_on), jnp.array(n_off))
    list_result = expected_passengers_per_station(n_on, n_off)
    assert jnp.allclose(list_result, arr_result)
