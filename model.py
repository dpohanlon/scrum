...
def compute_alphas(n_on, n_off):
    ...
    """
    num_rounds = len(n_on)
    population_before = jnp.zeros(num_rounds+1)  # station 0..num_rounds; station i is before i-th alighting
...
def precompute_survivors(n_on, alpha):
    ...
    # But let's keep it simpler (and more direct) for illustration.
    ...
    for r in range(num_rounds):
        for k in range(num_rounds):
            if k >= r:  # round r boards at station r, so r must be <= k to exist at station k
...
def passenger_location_model(n_on, n_off, positions_on):
    ...
    #    for demonstration. The user might pass it in as well.
...
def sample_passenger_locations_per_station(n_on, n_off, positions_on, path, num_samples=1000, seed=0):
    ...
    for i in range(num_rounds):
        passenger_locations_per_station[path[i+1]] = samples[f"passenger_location_{i}"]
