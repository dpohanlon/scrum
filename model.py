...
def compute_alphas(n_on, n_off):
    ...
    # ``jnp.result_type`` cannot operate directly on traced values inside a
    # ``jit``-compiled function.  Converting the inputs to JAX arrays first
    # exposes their static ``dtype`` attributes, allowing us to compute a
    # common dtype without triggering tracing errors.
